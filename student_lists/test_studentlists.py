...
class TestStudentLists(TestCase):
	...
	def test_for_error_when_removing_empty_list(self): #tests to make sure you can't remove things from empty lists
		test_class = ClassList(2)
		test_class.remove_student('Test Student')
		with self.assertRaises(StudentError):
			test_class.remove_student('Test Student')
	...
	def test_none_when_empty(self): #tests to return none when list is empty
		test_class = ClassList(2)
		with self.assertRaises(StudentError):
			self.assertIsNone(test_class.index_of_student('Student1'))

	def test_none_when_student_not_present(self):  #tests to return none when student is not present
		test_class = ClassList(2)
	...
	def test_is_class_full(self): #tests if class is full
		test_class = ClassList(2)
		with self.assertRaises(StudentError):
			self.assertTrue(test_class.is_class_full)

	def test_is_class_full(self): #tests if class is not full
		test_class = ClassList(2)
		with self.assertRaises(StudentError):
